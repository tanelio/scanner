...
package main {
  ...
  class Rules(tag: Tag) extends Table[(Int, String, String, Int, Int, Int, Boolean)](tag, "RULES") {
...
//    Future.sequence(schema.create).onComplete()
    ...
    val setup = DBIO.seq(
      ...
      rules += (3, "^saslauthd.+do_auth.+auth failure.+\\[user=(\\w+)\\].+",
                   "^sm-mta\\[.+\\[$ipv4\\].+did not issue MAIL/EXPN/VRFY/ETRN during connection to MTA", 1, 0, 3600, true)
    )
